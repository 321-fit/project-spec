...
| *(semi-private)* | **Semi-private sessions (proposal)** — not a file: states of existing screens. Coach `calendar.html#s-calendar` drawer with the roster (`is-semi`), `#s-event-edit` with seats + extras (`sp-seats`, `event-seat-sheet`, `event-extra-sheet`); athlete `calendar.html#s-schedule` → `openAthEvent('reconfirm')` / `openAthEvent('request',{seat:true})`. Spec: [semi-private.md](../specs/semi-private.md). | per-seat money, add-a-person-to-a-booking |
