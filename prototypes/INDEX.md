...
| `profile-drafts.html` | **Profile & Settings rework (proposal)** — Settings + Availability hub redrawn as panels (week as seven bars); Profile, Personal info, Sports, Hours, Time off, Booking rules, Locations, Calendar sync, Stripe, Referral copied in under the tinted look. Built with `~/.claude/scripts/proto-build-rework.py` + a JSON config. | settings/status pages in the row-panel grammar |
