...
| `client-detail-drafts.html` | **Client Detail redesign — draft J (proposal, not shipped)** — the whole flow in one file: J (states in the annotation) · Owed · Self-paced per client · Packages per client · every canon screen J reaches copied in (scoped `src-*`) · calendar drawers in-phone · **Look switch: Plain / Tinted canvas + CTA Gradient / White** (default tinted + white). Spec: [client-detail-redesign.md](../specs/client-detail-redesign.md). Drafts A–I live in git history. | J's grammar: circles carry weight · Needs-you chips · card→drawer, no buttons · money row→list · tiles lead with a visual · tinted material |
| `dashboard-drafts.html` | **Coach Home redesign (proposal)** — Home in J's grammar (greeting as title, four circles, Needs-you chips, Next session card, Money widget, Today, signals, one tip) with 7 states · Cash to collect (global Owed) · Inbox / review queue / Earnings / Book ×4 / Time off copied in · same look switch. Spec: [dashboard-redesign.md](../specs/dashboard-redesign.md). | the same grammar applied to a hub screen |
| `clients-drafts.html` | **Coach Clients redesign (proposal)** — the Clients tab in J's grammar: Needs-you chips = the five smart groups (doors, not filters) · This week (rows say *Next · Thu 10:00*) · Everyone A–Z + index rail · search · fresh-coach card · Groups pane without smart groups; smart group / groups / archived / import / create copied in. Spec: [clients-redesign.md](../specs/clients-redesign.md). | a list ordered by weight; rows speak of time, not lifetime money |
| `calendar-drafts.html` | **Calendar under the rework look (proposal)** — canon calendar + drawers on the tinted canvas; fill decisions (no hatch anywhere) live in `fit-ui.css`. | a canon module viewed in the rework look |
| `profile-drafts.html` | **Profile & Settings rework (proposal)** — Profile redrawn as your own Client Detail; Settings + Availability hub as panels (week chart = toggleable proposal); Account access copied in; Profile, Personal info, Sports, Hours, Time off, Booking rules, Locations, Calendar sync, Stripe, Referral copied in under the tinted look. Built with `~/.claude/scripts/proto-build-rework.py` + a JSON config. | settings/status pages in the row-panel grammar |
| `sessions-drafts.html` | **Sessions & Packages under the rework look (proposal)** — canon module copied; template cards = one alpha surface, type in the icon plate. Reached from Profile → Coaching. | the booking-card rule applied to the catalogue |
| *(web · stand)* | **Web invite — book.321.fit (proposal)** — not an HTML file: React screens in `stand/book.html` (`src/book/`), on a mock API. Event page (generic / personal / 1:1, every state) → answer flow (who · phone · code · details · pay) → the living answer page. Cloned from `athlete-drafts.html#s-draft-agroup` (tiles). The `cd-*` grammar it uses is lifted into `lib/fit-ui-rework.css`. Spec: [web-invite.md](../specs/web-invite.md). | a public web surface built from the rework grammar |
| `athlete/athlete-drafts.html` | **Athlete side rework (proposal)** — the coach grammar mirrored: Home, My coaches, Coach detail, Profile, Settings, Training history, History with coach, Addresses, Home-visit picker, Group join redrawn; Schedule, Search (light, athlete-specific), public coach profile + booking, Balance, Personal info, Calendar sync, Integrations copied in. Config: `cfg-ath.json` next to `~/.claude/scripts/proto-build-rework.py`. | the same grammar on the other role |
| *(semi-private)* | **Semi-private sessions (proposal)** — not a file: states of existing screens. Coach `calendar.html#s-calendar` drawer with the roster (`is-semi`), `#s-event-edit` with seats, extras, the seat editor and the pre-save warning drawer (`sp-seats`, `event-seat-sheet`, `event-extra-sheet`, `event-save-warnings-sheet`), picker via `invite.html?mode=seat` (by-link row hidden); athlete `calendar.html#s-schedule` → `openAthEvent('reconfirm')` / `openAthEvent('request',{seat:true})`. Spec: [semi-private.md](../specs/semi-private.md). | per-seat money, add-a-person-to-a-booking, pre-save warnings |
...
| `widget.html` | **Home-screen widget (proposal, #43)** — small / medium / large for both roles on a mock home screen; brand vs system skin; iOS 18 tinted; lock-screen accessories; Android; states | any glanceable summary of the schedule |
...
> **Rework grammar (`cd-*`)** — identity block, chips, panels/rows, tiles, forms `cd-f-*`, pickers `cd-pick-*`, the tinted canvas `k-alpha` + white CTA `k-cta-white` — lives in **`lib/fit-ui-rework.css`** since 2026-09-17 (lifted from the drafts' heads; the drafts still carry their own copy). Grep it before adding a page-local `cd-*` rule.
