...
| `athlete/athlete-drafts.html` | **Athlete side rework (proposal)** — the coach grammar mirrored: Home, My coaches, Coach detail, Profile, Settings redrawn; Schedule, Search (light, athlete-specific), public coach profile + booking, Balance, Personal info, Calendar sync, Integrations copied in. Config: `cfg-ath.json` next to `~/.claude/scripts/proto-build-rework.py`. | the same grammar on the other role |
