...
- **48h response window** — both athlete-requested and coach-scheduled requests auto-expire if not actioned within 48h (auto-declined → refund). Matches `event-statuses.md` (48h auto-cancel). Configurable per coach in future. *(Updated from 24h → 48h, 2026-06-26.)* **The effective deadline is `expires_at = min(submitted_at + 48h, datetime_start − 2h)`** — a short-lead request closes before the session, not at it — and two reminders fire before it lapses. *(2026-09-01, see [notifications-catalog.md](./notifications-catalog.md) § 1.3.)*
...
- **Pack credit — a third pay path** *(new 2026-07-15, see [session-packages.md](./session-packages.md))*. If the athlete holds credits for **this session template with this coach**, the confirm sheet offers **"Use 1 session from your pack"** instead of Cash/Balance: the price is struck through → "Included in pack", the CTA becomes **"Book · use 1 session"**, and a **"Pay per session instead"** link falls back to the normal chips. **Nothing is charged or reserved at booking** — the pack was paid upfront; booking **burns 1 credit** (FIFO across that athlete's lots). Everything else about the booking is unchanged: it is still a request, still needs the coach's approval, still respects the 48h window.
  - **Backward-compatible:** the booking-confirm endpoint is **extended additively** with an optional "pay with pack credit" field. Clients that don't send it behave exactly as today.
  - **Credit is returned** whenever the money path would have refunded: coach declines, coach cancels, athlete cancels per policy, or the 48h window expires. See `event-statuses.md` § 7.
  - A cash pack that the coach hasn't marked received is **still bookable** — the coach's approval is the control, so gating redemption too was redundant friction.
- **Cancellation policy** — athlete: free cancel up to 24h before session; coach: must cancel via Calendar (different flow). Cash sessions: no reservation, no refund logic; coach marks paid post-session. **Pack-funded sessions follow the same rules** — the pack changes *what pays*, not *how booking behaves*; where a card session would be refunded, a credit is returned instead.
