...
- **Coach wizard (6 steps)** — gated onboarding; all 6 steps required before profile is submitted for admin review and becomes visible to athletes.
...
- As a **coach**, I want a clear progress indicator (1 of 6 completed, 17%) so that I can see how close I am to done.
...
- As a **coach finishing all 6 steps**, I want the wizard to automatically submit my profile for review and transition my dashboard so that I don't need to manually "submit" anywhere.
...
- As the backend, when all 6 steps show done → the profile auto-transitions to `pending_admin_approval` without needing an explicit client-side "submit" call.
...
3. Wizard widget:
   ...
   - Progress: "0 of 6 completed · 0%" + progress bar
   - 6 step rows (gradient-tinted, all incomplete initially)

### Flow 2: Step completion

1. Coach taps step row (e.g., "Add phone number")
2. Navigates to the relevant screen (Account Access → Phone flow)
3. Completes the step, returns to dashboard
4. Dashboard re-fetches snapshot → wizard now shows `phone` step as done:
...
5. Progress indicator updates: "1 of 6 completed · 17%", bar fills proportionally
...
1. Coach completes the last of 6 steps
2. On return to dashboard, wizard shows 6/6 · 100%, all rows checked
...
1. Athlete signs up, lands on Athlete Home → sees wizard (4 steps: phone, sports, photo, find coach)
...
| (never `disabled` — all 6 are always relevant) |
  ...
  "steps": [
    {
      ...
      "done":         true,
      "deeplink":     "/shared/account-access#s-phone-new",
      "estimatedMin": 1
    },
    {
      "key":          "sports",
      ...
      "done":         false,
      "deeplink":     "/coach/settings#s-sport-types",
      "estimatedMin": 1
    },
    {
      "key":          "photo",
...
Each step's `done` is computed from the actual profile state (e.g., `phone.done` = `coach.phone_verified`, `session.done` = `coach.training_sessions_count > 0`). No separate "completed" flag on the wizard — no drift possible.
