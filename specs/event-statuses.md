...
- **A hair of blur (0.8px) on top, as progressive enhancement.** The hour labels sit *under* the zone so they fall inside both the wash and the blur, and the band recedes as one. Their opacity is raised to pay for the wash above them — 0.75 in dark (35% black on top; 0.55 measured too faint), 0.6 in light (~10% wash, so it keeps the muted look).
- The blur is **never the load-bearing signal**: the faded rules and recessed edges carry the meaning, so where it doesn't render — Android below API 31 has no `Modifier.blur` — nothing is lost. Radius stays under 1px for a reason: at 1.6px a 10px label like "04:00" smudges into an unreadable blob, and the coach still navigates by those labels.
- Every contiguous band repeats the short label ("Outside your hours", **12px 500 tertiary** — the caption step, not the 11px pill step: it explains a whole band and at 11px under the blur it read as debris; still below the 13px tile title so it never competes with an event). No time range in it — the ruler already shows that. A fragmented availability day therefore explains itself wherever the user lands. Bands under 32px drop the label and keep the wash.
