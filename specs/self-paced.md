...
- **Assign is a footer CTA** on `#s-client-selfpaced` ("Assign self-paced"). Offering already known (chip on, or the coach sells one) → **straight into the builder**; otherwise an **offering picker sheet** first (tap-to-act rows with the price, no radio, no confirm — one decision, then the builder; the day is asked for **in** the builder, not twice). Client Detail's *Assign self-paced* row lands here with that sheet open, so cancelling leaves the coach on a useful screen. The builder shows an **offering context chip** (`?offering=`) — it is always known and it sets the price — and its back button returns to the door used (hub vs client list).
  - **Why a sheet and not the templates catalogue.** `sessions.html#s-list` is a *management* screen — tap, detail, edit — not a picker; making it one changes what it means and navigates the coach away from the client they are mid-task on. The realistic range is 1–3 offerings (§8: one generic offering for v1), and at exactly one the sheet never appears.
  - **It still has to survive N.** Rows are generated from the offering list and the list scrolls; past **six** an always-visible *See all N self-paced* row hands off to `sessions.html?only=selfpaced#s-list` — the catalogue filtered to this type, with the Sessions/Packages segmented control and the add button suppressed, because picking is not browsing. That row sits **outside** the scroller: an overflow door is useless if you must scroll past the overflow to find it.
  - **A *New self-paced offering* row** opens `sessions.html?new=selfpaced#s-create` with the training type pre-selected. Without it, a coach who realises mid-assign that they need a new offering is in a dead end.
