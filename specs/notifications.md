...
> Component library: [design-tokens/docs/components.md](https://github.com/321-fit/design-tokens/blob/main/docs/components.md) — FitNotificationRow, FitIconBtn, FitIconPlate, FitEmptyState, FitSkeleton, FitSnackbar, FitSegmented
...
The full list of every notification the app sends — push body, trigger, channels, routing, required `template_data` — lives in a dedicated companion doc:

**→ [`notifications-catalog.md`](./notifications-catalog.md)** (single source of truth — copy + style guide + variables vocab + all **40** backend categories)

This split was made on 2026-05-22 because the catalog grew to ~50 lines and was hard to find buried between Routing and Inbox UI sections. The companion doc is the **primary reference** for "what does the app send"; this doc focuses on the infrastructure that delivers it.

When changing notification copy or adding a new category:
1. Update [`notifications-catalog.md`](./notifications-catalog.md) first
2. Update `notification_template.push_text` via Alembic migration
3. Verify the call site passes all `template_data` keys listed in the catalog row

The **kit type** column in the catalog maps to client-side rendering + tap routing — see [§ Notification Inbox UI § Type → icon / color mapping](#type--icon--color-mapping-kit-type-enum-decoupled-from-backend-targetroute) and [§ Tap routing](#tap-routing-sheet-vs-push) below.

---

## Inbox UI (unified 2026-06-02)

The Inbox is the single in-app surface for everything that touches the coach's queue: notification history, incoming booking requests from athletes, and outgoing invites the coach sent and is waiting on.

> **2026-06-02 unification.** Previously the coach had *two* bell icons (Dashboard + Clients header) leading to *two* inboxes — `s-notifications` for push history and `s-requests` (Clients tab) for incoming requests. This was confusing ("which bell?") and didn't scale to outgoing pending. Both bells collapsed into **one** bell (Dashboard), which now opens an Inbox with **3 tabs**. The Clients bell + `s-requests` screen + sidebar entry are deleted. Pending requests are a flavor of notification, not a separate category.
...
| **Dashboard pair action cards** | Home tab "Needs your attention" section | "3 pending requests" + "2 invites awaiting reply" — deep-link via `openInboxOnTab('toreply' \| 'waiting')` |

All three primary badges pull from `GET /notifications/count` for the unread Activity feed PLUS the incoming pending count (`GET /coach/training-events?status=athleteRequested`). **Outgoing pending (Waiting tab) is NOT counted toward badge** — bell signals "action needed by you"; monitoring others is surfaced via the tab counter + Dashboard pair card.

Show `1`–`99`; collapse to `99+` when ≥100; hide entirely when 0. Refresh on screen mount, app-foreground (`onAppear` / `LifecycleObserver.onStart`), and immediately after every mark-read / Accept / Decline action.
...
- **Header** — back chevron (left) · "**Inbox**" title (center) · `FitIconBtn` ⋯ menu (right). The ⋯ menu has one item: **Mark all as read** (disabled when 0 unread; only acts on Activity tab content).
- **Tabs** — segmented control (`FitSegmented`) below header. Three options with live count badges: **Activity (N)** · **To reply (M)** · **Waiting (K)**. Default tab on entry = **Activity**. State persists during the session but does not survive cold start (always opens to Activity).
- **Content** — tab-scoped panel rendered below the segmented control. See § Tab content below.
- **Footer** — none (no tab bar on this screen per `feedback_navbar_visibility` — it's a pushed screen, not a tab root).

### Tab content

**Activity (default)** — historical push-triggered notification feed (unchanged from pre-unification). Scrollable `FitNotificationRow`s grouped by date. Section headers: **Today** · **Yesterday** · **Earlier this week** · month names ("April", "March"). 12pt 500 uppercase `text-secondary`. Pull-to-refresh + infinite scroll (paginated `GET /notifications`).

**To reply (incoming pending booking requests)** — list of `RequestCard`s, one per pending `training_event` where `status = athleteRequested` and the coach is the recipient.

- Card content: avatar + athlete full name + training session name + date/time + location + price + payment type badge.
- Actions: **[Decline] [Accept]** (44pt buttons, destructive variant + brand gradient respectively).
- Tap on avatar row → push to `s-client-detail`.
- Accept → optimistic fade-out, snackbar "N accepted — event added to calendar", event moves to `confirmed`, athlete added to clients if new. Bell badge + tab counter decrement.
- Decline → optimistic fade-out, snackbar "N's request declined", event → `declined`. Badge + counter decrement.
- Last row handled → inline `FitEmptyState` "Inbox zero — No requests right now" within the panel.

**Waiting (outgoing pending coach → athlete invites)** — list of `RequestCard`s with status surface, one per `training_event` where `status = coachInvited` and the athlete hasn't responded.

- Card content: avatar + athlete full name + **"Awaiting reply"** pill (yellow dot + 12pt) + "sent N ago" + training session name + date/time + location + price + "Expires in N" footer.
- Actions: **[Edit] [Cancel]**.
  - **Edit** — push to `s-event` in edit mode with status `coachInvited`. Coach can change date/time/location/note/price. Save fires `PATCH /training-events/{id}` which re-pings the athlete (push notification "Coach updated invite — please confirm"). Replaces the previously considered Resend action: Edit covers the "nudge the athlete" intent (the re-ping is the side effect) and also handles the "actually I want to change details" intent (the primary effect).
  - **Cancel** — destructive `POST /training-events/{id}/cancel`. Athlete gets push "Coach cancelled invitation". Optimistic fade-out, snackbar "Invite to N cancelled", tab counter decrement.
- Tap on card body (not on buttons) → push to read-only `s-event` detail with "Awaiting reply" status badge.
- Last row handled → inline `FitEmptyState` "No invites pending. When you invite athletes to a session, their pending replies show up here." within the panel.

### Tab counter logic

| Tab | Counter source | Updates on |
|---|---|---|
| **Activity** | unread `notifications` count from `GET /notifications/count` | mark-read, mark-all-read, new push arrival |
| **To reply** | incoming pending requests count from `GET /coach/training-events?status=athleteRequested` (or new dedicated `/coach/pending-requests/count`) | Accept, Decline, new request arrival |
| **Waiting** | outgoing pending invites count from `GET /coach/training-events?status=coachInvited` (or new dedicated `/coach/pending-invites/count`) | Edit (no count change — invite stays pending), Cancel (decrement), athlete responds (decrement), new invite created (increment) |

### Empty states per tab

| Tab | Trigger | UI |
|---|---|---|
| **Activity** | `GET /notifications` returned 0 records | `FitEmptyState` bell illustration · "No notifications yet" · no CTA |
| **To reply** | 0 incoming pending | `FitEmptyState` inbox-zero illustration · "Inbox zero — No requests right now" · no CTA |
| **Waiting** | 0 outgoing pending | `FitEmptyState` paper-plane illustration · "No invites pending. When you invite athletes to a session, their pending replies show up here." · no CTA |

**Screen-wide loading + error states** wrap all three panels (single fetch state — the 3 counts are fetched in parallel during inbox mount).
...
Backend ships **40** `NotificationCategory` values (see [notifications-catalog.md § 1](./notifications-catalog.md)); the kit collapses them to 13 visual variants:
...
| `reminder` *(new 2026-05-22)* | `trainingSoon`, `cashOverdue` | bell · `FitIconPlate(.info)` |
...
> **Reminder = `training_soon`, not a 1h/10min split (2026-07-17).** The `reminder` kit type is fed by the shipped `training_soon` category (Celery beat fires a single push ~10 min before an event, to **both** roles) plus `cash_overdue`. The earlier `session_reminder_1h` / `session_reminder_10min` split is **phantom** — the enum values exist and are wired into clearance, but no producer creates them. See [notifications-catalog.md § 1 #12–13](./notifications-catalog.md#1-the-catalog--40-backend-categories).
...
| `request` | **Push** → Home tab → Inbox (`s-notifications`) with **To reply** tab pre-selected | Updated 2026-06-02 after bell unification — `s-requests` no longer exists. Routing now matches dashboard's "Pending requests" action card which calls `openInboxOnTab('toreply')`. Aggregate list view; sheet doesn't fit a multi-card screen. |
...
| `reminder` | **Sheet over inbox** — `FitUI.openEventSheet(event:)` for `trainingSoon`; **Push** → Earnings → `s-txn-cash` for `cashOverdue` | Reminders point to a single event (sheet pattern matches `approved` / `cancelled`). `cashOverdue` routes to the cash transaction detail so coach can act (Mark as paid). |
