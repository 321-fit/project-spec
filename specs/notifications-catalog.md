...
| 11 | `training_session_successful_athlete` | `approved` | Session completed for athlete | P · I | **Session complete** | `{session_name} with {coach_name} on {date} completed. Leave a review?` | Athlete → Schedule (sheet, finished state) — surfaces Leave Review CTA | tap | `session_name, coach_name, date` |
