...
> **v2 redesign exploration (2026-06-26) — `athlete/balance-v2.html` (parallel module, original `balance.html` kept for rollback).** Revolut-style "money dashboard": (1) a single **Balance card** on top (no Spending swipe-card); (2) a 2×2 **analytics widget** grid — *This month* (with card/cash split), *Total spent* (lifetime), *Booked* (planned spend = upcoming sessions → its own **Booked** list), *Owed* (red, cash debt → its own **Owed** list); (3) a **Recent** 3-row preview + **See all**; (4) the full ledger moves to its own **History screen** (`s-history`) with a **sticky** search + filter chips by *type* (Spent / Top-ups / Refunds) **and payment method (Card / Cash)** — the list scrolls underneath. Transaction rows read **coach name** as the title + a Card/Cash method pill + session name as the subtitle (matches the coach ledger). Not yet promoted to canonical — pending review.
>
> **v2 detail polish (2026-06-26).** (a) **Session names** read as sessions, not bare sports — "1:1 Tennis / Boxing / Padel". (b) **Top-up rows show time** in the meta line. (c) **Auto top-up ON** → a compact "↻ Auto €50" chip top-right of the Balance card header + the footer note shows the threshold ("Tops up below €10"). (d) **Transaction detail screens** — a Card/Cash **method pill** sits top-right of the hero; the hero subtitle is the **charge/confirm timestamp** ("Charged … · 19:05") while the Session section carries a **"When"** row with the actual **training time** ("… · 18:00"). (e) **Cash sessions open their own detail** (`s-txn-spend-cash`) — *not* the card one. Cash detail shows the Cash pill and **no** "Paid from / Balance after" (money isn't taken from balance); status is **Paid** (teal, canonical `fit-badge-accent`) — a cash session appears in the ledger **only once the coach confirms receipt**. **Owed (not-yet-paid) cash sessions never appear in the balance ledger** — they're surfaced on the Dashboard, coach detail, and the red *Owed* summary widget. (f) **Refund detail has 3 reasons** — *Coach cancelled* / *You cancelled* / *Auto-declined* (coach didn't confirm the request within **48h** → request expires, auto-refund), each with a contextual outline note.
>
> **v1 hidden (2026-06-26):** the original `balance.html` (athlete) + `coach/balance.html` (Earnings) hub cards are commented out in the prototype index to avoid confusion; files kept on disk for rollback. v2 is the only Balance/Earnings entry shown.
>
> **v2 widget drill-downs + period scope (2026-07-13) — `athlete/balance-v2.html`.** Each analytics widget now lands on the *itemised* thing behind its number, not a generic screen. Two new dedicated list→detail surfaces, plus a period scope on History:
> - **Owed** (`s-owed` → `s-owed-detail`) — the red widget lists the completed **cash** sessions the athlete still owes for (canonical `.fit-txn` rows: coach + Cash pill · session · when · **red** amount; `fit-txn-icon--danger`), under a warn `.fit-detail-hero` total. **Owed cash sessions never hit the balance ledger** (no wallet movement until the coach confirms receipt). No athlete-side "Mark paid" — the coach confirms (see Flow J). Detail: Status = **Owed** (red), pay-in-person note; the **Coach** KV row is tappable → coach detail (`my-coaches.html#s-coach-detail`); footer CTA **Message coach** → `messages.html` (real job = coordinate the cash hand-off).
> - **Booked** (`s-booked` → `s-booked-detail`) — the widget lists **upcoming, already-paid** sessions whose funds are **held** from balance (blue/info: `fit-txn-icon--info` + `fit-badge-info`, neutral amounts). Mirror of Owed but nothing is wrong — it's planned spend. Detail: Status = **Held until session**, Paid from Balance, hold/refund note; Coach row → coach detail; footer CTA **View in calendar** → `calendar.html` (reschedule/cancel live on the event, not here — no duplicate cancel path).
> - **History (`s-history`) — quick type chips + a Filter screen.** The ledger keeps a single quick-chip row (**type**: All / Spent / Top-ups / Refunds, one-tap). Everything else moves behind a **funnel** in the search bar (green dot when a filter is set) → a push **Filter screen (`s-history-filter`)** built on the **canonical Search-filter kit** (`.filter-section` / `.filter-chip` / `.filter-sticky-cta`; header **Reset**, sticky **Apply**, draft-then-commit). It holds **Period** — presets *All time / This week / This month / This year* + **Custom…** (From/To range). **No payment-method filter**: in this ledger method mostly overlaps type (top-ups + refunds are always card; only *spends* split card/cash) and cash reconciliation lives on the *Owed* screen, so it earned no section. Widget deep-links: **This month** → period = month + type Spent (funnel dot lit; total matches the €245 tile); **Total spent** → all-time + Spent. Presets resolve to a `from`/`to` range. The prototype filters client-side for the demo; **native resolves all History filters server-side** (`type` + `from`/`to` on `GET /athlete/transactions-history`), consistent with the Search screen — pass a `{ from, to, type, search }` query, don't filter a client copy. (Earlier iterations tried a lone period toggle chip, then a Filter screen with a Payment-method section — both dropped.)
> - **Consistency rule (recorded):** tap a widget → if the user wants to *act on specific items* (pay / message / manage), it gets a **dedicated list screen** (Owed, Booked); if they want to *see what makes up an aggregate*, it deep-links the **filtered ledger** (This month, Total spent). No bespoke screen for a number that is just summed transactions.
...
> **Cash spends** enter the ledger as `spend` (Cash method pill, status **Paid**) but do **not** debit the balance — no "Paid from / Balance after". A cash session is added only after the coach marks it paid; **owed** (unconfirmed) cash sessions are not in the ledger — they live on the Dashboard, coach detail, and the *Owed* widget.
