...
> - **Period scope on History (`s-history`)** — a **This month** toggle chip (`.hist-period` row) that is an **independent second axis**, combining with the existing type (Spent/Top-ups/Refunds) + method (Card/Cash) chips; rows carry `data-month`. It is a **toggle, not a removable ×-chip** (per `filter_chips_no_inline_remove`). Widget deep-links set both axes at once: **This month** → month scope ON + Spent (drill-down total matches the €245 tile); **Total spent** → all-time + Spent. The prototype filters client-side for the demo; **native resolves all History filters server-side** (type + method + `from`/`to` date range on `GET /athlete/transactions-history`), consistent with the Search screen where every filter is backend-driven — pass a `{ from, to, type, method, search }` query, don't filter a client copy.
