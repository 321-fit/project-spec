...
Getting around: the left panel is **role → module → screens**, in flow order, each screen with
its status dot, a `+N` states count and an orphan flag. Depth is not repeated as headers here —
it is already the canvas's own axis. Roles and modules fold independently, everything starts
folded, `Collapse all` / `Expand all` sit at the top of the panel, and the state is remembered
between sessions; `\` shrinks the whole panel to a **46px rail** of module chips
